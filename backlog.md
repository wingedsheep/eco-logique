...
- `./gradlew build` succeeds on empty project
...
**Acceptance Criteria**:
- `docker-compose.yml` starts PostgreSQL with separate schemas
- `docker-compose.yml` starts RabbitMQ
- Init script creates schemas: payment, products, shipping, inventory, users
- Can connect to databases from host machine
- README documents how to start environment
...
**Acceptance Criteria**:
- ADR template created in `docs/architecture/decisions/`
- **ADR-001**: Use Modular Monolith Architecture
    - Context: Need to balance simplicity with modularity
    - Decision: Build modular monolith, not microservices
    - Consequences documented
- **ADR-002**: Apply Domain-Driven Design
    - Context: Complex business domain with multiple bounded contexts
    - Decision: Use DDD with bounded contexts per module
    - Consequences documented
- **ADR-003**: Separate Database Schemas per Bounded Context
    - Context: Need data isolation between modules
    - Decision: Use separate PostgreSQL schemas
    - Consequences documented
- **ADR-004**: Mappers in Persistence Layer
    - Context: Database entities should not leak outside persistence
    - Decision: Entity mappers stay in persistence package
    - Consequences documented
- **ADR-005**: Event-Driven Communication Between Modules
    - Context: Need loose coupling for async operations
    - Decision: Use domain events with Spring ApplicationEventPublisher
    - Consequences documented
