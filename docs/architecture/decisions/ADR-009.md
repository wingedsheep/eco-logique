...
Contains the Domain Model, Value Objects, Domain Events, Output Port Interfaces (Repositories), and **Interfaces for exposed Application Services**.
...
**`application/service/ReduceStockService.kt`** (Exposed Use Case Interface)
```kotlin
interface ReduceStockService {
...
**`application/service/ReduceStockServiceImpl.kt`**
```kotlin
@Service
class ReduceStockServiceImpl(private val repository: InventoryRepository) : ReduceStockService {
    @Transactional
    override fun execute(sku: Sku, amount: Quantity): Result<InventoryItem, InventoryError> {
